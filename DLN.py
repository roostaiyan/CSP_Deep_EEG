...
from tensorflow.keras import regularizers  # for sparsity constraint (Stacked Sparse Auto-Encoder
...
class DLN:
    ...
    def __init__(self, structure: np.array, rho: float, beta: float, dataset: data_handling.Dataset):  # Take DLN with a structure of 100(should be exactly equal to n)-80-100-80-100 for example,
    ...
    def train(self,n_epochs: int, learning_rate: float):
        ...
        history = self.stacked_AEs.fit(data_train_DLN_n, data_train_DLN_n,
                                       epochs=self.n_epochs,
                                       batch_size=bs,
                                       shuffle=True,
                                       validation_split=0.2)  # this will be used to validate model during training
